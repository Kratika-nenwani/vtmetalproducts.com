...
@section('content')
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h2 class="modal-title" id="sign_up_popupLabel2">Add categories</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="twm-tabs-style-2">
                            <form id="f" action="{{ 'save-categories' }}" method="POST"
                                                ...
                                                <div class="form-group mb-3">
                                                    <label for="image"><b>Image<span class="text-danger">
                                                                *</span></b></label>
                                                                ...
                                                                class="form-control my-1" multiple>
    ...
    @foreach ($data as $categories)
        <div style="width:100%;" class="modal fade twm-sign-up" id="editModal{{ $categories->id }}" tabindex="-1"
            role="dialog" aria-labelledby="exampleModalLabel{{ $categories->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel{{ $categories->id }}">Edit Event</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <!-- Form for editing blog -->
                        <form id="editForm{{ $categories->id }}"
                            action="{{ route('update-categories', ['id' => $categories->id]) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="tab-content" id="myTab2Content">
                                <!--Login Candidate Content-->
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-12">
                                            <div class="form-group mb-3">
                                                <label for="description"><b>Description<span class="text-danger">
                                                            *</span></b></label>
                                                <textarea type="text" class="form-control" name="description" id="edescription" rows="5">{{ $categories->description }}</textarea>
                                            </div>
                                        </div>
...
@section('jscontent')
    ...
    <script type="text/javascript">
        $(document).on('click', '.delete', function() {
            ...
            var url = "{{ route('delete-categories') }}";
            swal.fire({
                title: "Are you Sure?",
